class SeriesController < ApplicationController
  ...
  def index
    ...
    @Series = if params[:Titulo]
                Serie.where('"Titulo" ILIKE ?', "%#{params[:Titulo]}%").where(Estatus: 'Activa')

              elsif params[:Categoria_id]

                Serie.where(Estatus: 'Activa').where('"Categoria_id" = ?', "#{params[:Categoria_id]}").paginate(:page => params[:page], :per_page => 16).order(:id)
              else

                Serie.where(Estatus: 'Activa').paginate(:page => params[:page], :per_page => 16).order(:id)
              end
  ...
  def show
    ...
    begin
      @Serie = Serie.where(Estatus: 'Activa').find(params[:id])
    ...
    # if @Serie.nil?
    #   redirect_to "google.com" # action: "index"
    # end
